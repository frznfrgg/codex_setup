...
`$plan` may load this skill as a lazy module when a plan replaces, removes, migrates, deprecates, or consolidates behavior, APIs, data, dependencies, or systems.

When used from `$plan`, record compatibility, migration, rollout, rollback, and removal decisions in `.plans/PLAN_*.md`. When used from `$split-to-tasks`, validate that migration requirements are represented as concrete subtasks or review gates.
