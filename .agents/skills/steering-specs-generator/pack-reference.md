...
2. **Explore agents analyze the codebase** and produce shared discovery reports
3. **Question-prep agents generate dynamic questions in parallel** using:
   - Findings from discovery reports (what exists)
   - Topics from pack guidance (what to ask about)
4. **The parent workflow conducts the interview sequentially** with the prepared questions
